      ...
      <div className="container-page py-16 grid gap-10 md:grid-cols-4">
        <div className="md:col-span-1">
          ...
          <p className="mt-3 text-sm text-muted-foreground leading-relaxed max-w-xs">
            Public University Students' Association of Bishwambarpur — a non-profit, non-political association of 300+ students.
          </p>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2 text-sm">
            {PROGRAMS.slice(0, 6).map((p) => (
              <li key={p.key} className="text-foreground/80">{p.title}</li>
            ))}
          </ul>
        </div>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm">
            <li className="flex items-start gap-2.5"><Mail size={15} className="mt-0.5 text-[var(--color-accent-1)]" /><a href={`mailto:${SITE.email}`} className="hover:underline">{SITE.email}</a></li>
            <li className="flex items-start gap-2.5"><Phone size={15} className="mt-0.5 text-[var(--color-accent-1)]" />{SITE.phone}</li>
            <li className="flex items-start gap-2.5"><MapPin size={15} className="mt-0.5 text-[var(--color-accent-1)]" />Bishwambarpur, Sunamganj</li>
            <li>
