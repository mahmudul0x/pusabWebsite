      ...
      <section className="pb-24">
        <div className="container-page">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-8">
            ...
            <button onClick={signOut} className="inline-flex items-center gap-2 text-sm rounded-full glass px-4 py-2">
              <LogOut size={14} /> Sign out
            </button>
          </div>

          {isAdmin === null ? (
            <div className="rounded-3xl border border-border bg-[var(--color-surface)] p-10 text-muted-foreground">Loading…</div>
          ) : !isAdmin ? (
            <div className="rounded-3xl border border-border bg-[var(--color-surface)] p-10">
              ...
              <p className="text-sm text-muted-foreground">
                Your account has been created. Ask a site admin to grant your account the <code>admin</code> role,
...
function MomentsAdmin() {
  ...
  return (
    <div className="grid lg:grid-cols-[400px,1fr] gap-8">
      <form onSubmit={submit} className="glass rounded-3xl p-6 space-y-4 border border-border h-fit">
        ...
        <button disabled={busy} className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-[linear-gradient(120deg,var(--color-accent-1),var(--color-accent-2))] text-white font-semibold py-3 disabled:opacity-60">
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {items.map((it) => (
          <div key={it.id} className="rounded-2xl border border-border bg-[var(--color-surface)] overflow-hidden group">
            <img src={it.image_url} alt={it.title ?? ""} className="w-full h-44 object-cover" />
            <div className="p-4">
              <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">{it.category} · {it.year ?? "—"}</div>
              <div className="font-medium mt-1">{it.title ?? "Untitled"}</div>
              {it.caption && <div className="text-xs text-muted-foreground mt-1 line-clamp-2">{it.caption}</div>}
              <button onClick={() => remove(it)} className="mt-3 inline-flex items-center gap-1 text-xs text-red-400 hover:text-red-300">
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const { error } = await supabase.from("publicity_posts").insert({
        title, type, date, excerpt: excerpt || null, body: body || null,
        image_url, storage_path, published: true,
      });
      if (error) throw error;
      toast.success("Post added");
      setTitle(""); setExcerpt(""); setBody(""); setFile(null);
      await refresh();
    } catch (err) {
  ...
  return (
    <div className="grid lg:grid-cols-[400px,1fr] gap-8">
      <form onSubmit={submit} className="glass rounded-3xl p-6 space-y-4 border border-border h-fit">
        ...
        <Field label="Excerpt"><input value={excerpt} onChange={(e) => setExcerpt(e.target.value)} className={inputCls} /></Field>
        <Field label="Body"><textarea rows={5} value={body} onChange={(e) => setBody(e.target.value)} className={inputCls} /></Field>
      ...
      <div className="space-y-3">
        {items.map((it) => (
          <div key={it.id} className="flex gap-4 rounded-2xl border border-border bg-[var(--color-surface)] overflow-hidden">
            {it.image_url && <img src={it.image_url} alt="" className="w-40 h-28 object-cover shrink-0" />}
            <div className="p-4 flex-1 min-w-0">
              <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">{it.type} · {it.date ?? ""}</div>
              <div className="font-medium truncate">{it.title}</div>
              {it.excerpt && <div className="text-sm text-muted-foreground line-clamp-2">{it.excerpt}</div>}
              <button onClick={() => remove(it)} className="mt-2 inline-flex items-center gap-1 text-xs text-red-400 hover:text-red-300">
                <Trash2 size={12} /> Delete
              </button>
            </div>
          </div>
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (file) {
        ...
        photo_url = r.url; storage_path = r.path;
      }
      const { error } = await supabase.from("ec_members").insert({
        name, role, university: university || null,
        year: Number(year), is_current: isCurrent,
        photo_url, storage_path,
      });
      if (error) throw error;
      toast.success("Member added");
      setName(""); setRole(""); setUniversity(""); setFile(null);
      await refresh();
    } catch (err) {
      toast.error(err instanceof Error ? err.message : "Failed");
    } finally { setBusy(false); }
  ...
  return (
    <div className="grid lg:grid-cols-[400px,1fr] gap-8">
      <form onSubmit={submit} className="glass rounded-3xl p-6 space-y-4 border border-border h-fit">
        ...
        <Field label="Role"><input required value={role} onChange={(e) => setRole(e.target.value)} className={inputCls} /></Field>
        <Field label="University"><input value={university} onChange={(e) => setUniversity(e.target.value)} className={inputCls} /></Field>
        <div className="grid grid-cols-2 gap-3">
          <Field label="Year"><input type="number" value={year} onChange={(e) => setYear(e.target.value)} className={inputCls} /></Field>
        ...
        <Field label="Photo"><input type="file" accept="image/*" onChange={(e) => setFile(e.target.files?.[0] ?? null)} className="text-sm" /></Field>
        <button disabled={busy} className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-[linear-gradient(120deg,var(--color-accent-1),var(--color-accent-2))] text-white font-semibold py-3 disabled:opacity-60">
      ...
      <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
        {items.map((it) => (
          <div key={it.id} className="rounded-2xl border border-border bg-[var(--color-surface)] overflow-hidden">
            {it.photo_url ? (
              <img src={it.photo_url} alt={it.name} className="w-full h-48 object-cover" />
            ) : (
              <div className="w-full h-48 grid place-items-center text-muted-foreground text-sm bg-white/[0.02]">No photo</div>
            )}
            <div className="p-4">
              <div className="text-xs uppercase tracking-[0.18em] text-muted-foreground">{it.role} · {it.year}{it.is_current ? " · current" : ""}</div>
              <div className="font-medium mt-1">{it.name}</div>
              {it.university && <div className="text-xs text-muted-foreground mt-1">{it.university}</div>}
              <button onClick={() => remove(it)} className="mt-3 inline-flex items-center gap-1 text-xs text-red-400 hover:text-red-300">
                <Trash2 size={12} /> Delete
              </button>
            </div>
...
const inputCls = "mt-2 w-full rounded-xl bg-[var(--color-surface)] border border-border px-3 py-2 text-sm outline-none focus:border-[var(--color-accent-1)]";
